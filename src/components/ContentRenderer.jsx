...
import { getPageACSCodes } from '../data/acsMapping';
...
function ContentRenderer({ sections, glossaryData, currentPage }) {
  ...
  const { highlights, addHighlight, deleteHighlight, toggleRightSidebar, showACSBadges } = useAppStore();
  ...
  return (
    <div className="content-text" onMouseUp={handleTextSelection}>
      {sections.map((section, sectionIdx) => (
        <div key={sectionIdx}>
          {/* Render section title with auto-mapped ACS badges on level 2 */}
          {section.title && (() => {
            const explicitCodes = section.acs_codes || [];
            const mappedCodes = section.level === 2
              ? getPageACSCodes(currentPage).filter(c => !explicitCodes.includes(c))
              : [];
            const allCodes = [...explicitCodes, ...mappedCodes];
            const Tag = section.level >= 3 ? 'h3' : 'h2';
            return (
              <Tag>
                {section.title}
                {showACSBadges && allCodes.map((code) => (
                  <span
                    key={code}
                    className="acs-badge"
          ...
          {section.content.map((item, itemIdx) => {
            ...
            if (item.type === 'figure') {
              const chNum = currentPage ? currentPage.split('-')[0] : '1';
              const imgSrc = item.image
                ? `/images/ch${chNum}/${item.image}`
                : null;
              return (
